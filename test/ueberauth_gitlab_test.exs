defmodule UeberauthGitlabTest do
  ...
  alias Ueberauth.Strategy.Gitlab
  ...
  test "handle_request!" do
    use_cassette "handle_request!" do
      ...
      result = Gitlab.handle_request!(conn)
      assert result == nil
    end
  end
  ...
  describe "handle_callback!" do
    test "with no code" do
      conn = %Plug.Conn{}
      result = Gitlab.handle_callback!(conn)
